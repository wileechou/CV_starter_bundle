...
for imagePath in sorted(list(paths.list_images(args["dataset"]))):
    ...
    # extract the class label from the image path and updte the
    # label list
...
# directly use LabelBinarizer to gain one-hot code of labels
le = LabelEncoder().fit(labels)
labels = to_categorical(le.transform(labels), 2)

# account for skew in the labeled data
# for keras >=2.0, class_weight needs a dict
...
print(classification_report(testY.argmax(axis=1),
                            predictions.argmax(axis=1), target_names=labels.classes_))
